...
#define TRACE_LOG_PATH   GAME_HOME "/angrybirdas_load.log"
...
#define ENABLE_FMOD_PARTIAL_RELEASE_GUARD        1

/*
 * FMOD software-output cleanup guard.
 *
 * In this exact Unity 6000.3.5f2 libunity Build ID, FMOD's release path
 * dereferences object->system (+0x188) before checking it.  If an FMOD
 * allocation fails during construction (FMOD_ERR_MEMORY), that field can
 * legitimately still be NULL and the stock code crashes while unwinding.
 * The replacement prologue preserves x19/w20, checks +0x188, and skips to
 * the system-independent final cleanup when the system pointer is NULL.
 */
#define OFF_FMOD_PARTIAL_RELEASE_PROLOGUE         0x01961740u
#define OFF_FMOD_PARTIAL_RELEASE_NULL_CLEANUP     0x01961D30u

/* Unity aligned allocator fail-safe. A v0.1.67 OOM run returned 0x200010
 * from the region allocator and then touched ptr[-1]. Route NULL/very-low
 * results through the function's existing allocation-failure path instead. */
#define ENABLE_UNITY_LOW_ALLOC_GUARD                1
#define OFF_UNITY_LOW_ALLOC_CHECK                   0x0100BBB8u
#define OFF_UNITY_LOW_ALLOC_FAIL                    0x0100BC04u
#define UNITY_LOW_ALLOC_MIN_BYTES                   0x00400000u
...
#define DYNAMIC_HEAP_ALIGN_4K_INSN               0x52820008u
...
/* v0.1.71: disabled. The v0.1.70 hardware trace proved that shrinking
 * hbloader's process heap lowers InfoType_TotalMemorySize and UsedMemorySize by
 * the same amount, leaving the reported free margin unchanged (~3 MiB). It did
 * not create a separate Mesa/nouveau pool. Keep the experiment compiled but
 * inactive so the memory geometry is again controlled only by our sub-arenas. */
#define HOST_PROCESS_HEADROOM_BYTES (0ull)

/* The three Android images consume ~72.4 MiB total. 96 MiB leaves >23 MiB of
 * image-arena slack while returning 32 MiB to newlib versus the old 128 MiB. */
#define SO_ARENA_BYTES          (96ull * 1024ull * 1024ull)

/* Keep the black-box-free 33-slot geometry. v0.1.68 restores Unity's native
 * 64 MiB alignment contract and fixes the wrapper's trimmed-window ownership
 * bookkeeping instead of growing the pool or changing the kernel heap. */
...
/* v1.0.18 release: direct libnx audout is now the stable audio backend.
 * Disable the diagnostic load/audio trace again. Crash/fatal logging remains
 * independent and enabled through crash_log/fatal. main() removes any stale
 * angrybirdas_load.log when tracing is disabled. */
#define ENABLE_TRACE_LOG        0
#define ENABLE_LOAD_DIAGNOSTICS 0
#define TRACE_FILTER_AUDIO_ONLY 1
#define TRACE_FILTER_LEVEL_LOAD 0
#define TRACE_PUMP_INTERVAL_US  500000ull
#define TRACE_SLOW_IO_US        12000ull
#define TRACE_ASSET_IO_US       8000ull
#define TRACE_LARGE_IO_BYTES    (256u * 1024u)
#define TRACE_MAIN_BLOCK_US     100000ull
#define TRACE_PC_SAMPLE_START_US 1000000ull
#define TRACE_PC_SAMPLE_INTERVAL_US 200000ull
#define TRACE_WORKER_SAMPLE_INTERVAL_US 600000ull
#define TRACE_HANG_THRESHOLD_US 3000000ull
#define TRACE_HANG_REPEAT_US    3000000ull
#define TRACE_SLOW_FRAME_US     24000ull
#define TRACE_LARGE_TEXTURE_PIXELS (512u * 512u)
#define ENABLE_GFX_DIAGNOSTICS  0
#define ENABLE_ETC2_BC_TRANSCODE 1
#define GFX_RECLAIM_DELETE_THRESHOLD 16u
#define GFX_RECLAIM_PREALLOC_FORCE_THRESHOLD 8u
#define GFX_RECLAIM_ETC2_MIN_PIXELS (512u * 512u)
#define ZIP_IO_BYTES            (128u * 1024u)
#define INSTALL_SPACE_RESERVE_BYTES (64ull * 1024ull * 1024ull)

...
static int alog_buf_write(int b,int p,const char*t,const char*s){
    (void)b; (void)p; (void)t; (void)s; return 0;
}
...
static ssize_t nx_read(int fd,void *buf,size_t count){
    return fakefd_is_range(fd)?fakefd_read(fd,buf,count):read(fd,buf,count);
}
...
static size_t nx_fread_profile(void *ptr,size_t size,size_t nmemb,FILE *stream){return fread(ptr,size,nmemb,stream);}
static int nx_inflate_profile(z_streamp strm,int flush){return inflate(strm,flush);}
...
/* v0.1.69 graphics/memory tracing.
 *
 * Do not drain/replay GL errors and do not retry allocations here.  Those
 * diagnostics changed guest-visible glGetError timing and eventually filled
 * the replay queue.  Instead, remember the most recent instrumented operation
 * on each thread and log only when Unity itself calls glGetError.  This keeps
 * GL semantics byte-for-byte host-like while still correlating OOMs with the
 * texture request that immediately preceded them.
 */
typedef struct {
    uint64_t seq;
    const char *op;
...
static void gfxdiag_install_debug_callback(void){
    ...
    if(!g_real_glDebugMessageCallback)return;
...
static int gfxdiag_is_large_etc2_rgba(GLenum internalformat,GLsizei width,GLsizei height){
    if(internalformat!=0x9278u && internalformat!=0x9279u)return 0;
...
static GLenum nx_glGetError(void){return gfxdiag_host_error();}
...
static void nx_glCompressedTexImage2D(GLenum target,GLint level,GLenum internalformat,
    ...
    GFXDIAG_RUN_RETRY("glCompressedTexImage2D",
...
static void nx_glTexImage2D(GLenum target,GLint level,GLint internalformat,
    ...
    GFXDIAG_RUN_RETRY("glTexImage2D",
...
static void nx_glGenerateMipmap(GLenum target){
    ...
    GFXDIAG_RUN_RETRY("glGenerateMipmap",g_real_glGenerateMipmap(target));
}
...
static DynLibFunction g_core[] = {
 ...
 MAPN("fopen",nx_file_fopen_profile),MAP(fclose),MAPN("fread",nx_fread_profile),MAP(fwrite),MAP(fflush),MAP(fseek),MAP(ftell),MAP(fseeko),MAP(ftello),MAPN("fseeko64",fseeko),MAPN("ftello64",ftello),MAP(fgetc),MAP(fgets),MAP(fputc),MAP(fputs),MAP(fprintf),MAP(printf),MAPN("puts",nx_puts),MAP(vfprintf),MAP(vprintf),MAP(sprintf),MAP(snprintf),MAP(vsnprintf),MAP(vasprintf),MAP(fscanf),MAP(sscanf),MAP(vsscanf),MAP(fileno),MAP(fdopen),MAP(feof),MAP(ferror),MAP(clearerr),MAP(setbuf),MAPN("remove",file_remove),MAPN("rename",file_rename),

...
static void positioned_trace_note(const char *kind,int fd,off_t offset,size_t requested,
    ...
    int asset=path&&*path&&interesting_asset_path(path);
    if(elapsed>=TRACE_SLOW_IO_US || (asset&&(elapsed>=TRACE_ASSET_IO_US || requested>=TRACE_LARGE_IO_BYTES)) || result<0){
...
int file_fd_path(int fd,char *out,size_t cap){
    if(!out||!cap)return 0;
    out[0]=0;
    return positioned_copy_metadata(fd,NULL,out,cap);
}
...
ssize_t file_pread(int fd,void *buf,size_t count,off_t offset){
    ...
    ssize_t result=-1;
    int fallback=0;
    char block_detail[384];
    snprintf(block_detail,sizeof(block_detail),"fd=%d off=%lld req=%zu path=%s",fd,(long long)offset,count,path[0]?path:"?");
    trace_main_block_begin("io:pread",(const void*)(uintptr_t)fd,block_detail);

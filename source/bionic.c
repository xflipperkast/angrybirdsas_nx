...
static void guest_wait_begin(const char *kind,const void *key);
static void guest_wait_end(void);
static long bionic_sched_setaffinity(long tid,size_t cpusetsize,const void *guest_mask);
...
static ssize_t read_mapped_file_range(int fd, void *dst, size_t len, long off){
    ...
    size_t done=0;
    while(done<len){
        ssize_t r=file_pread(fd,(uint8_t*)dst+done,len-done,(off_t)off+(off_t)done);
...
void *bionic_mmap(void*a,size_t n,int prot,int flags,int fd,long off){
    ...
    if(file_backed){
        ...
        if(got<0){
            int e=errno;
            trace_log_printf("MMAP","file read failed fd=%d off=%ld bytes=%zu errno=%d time=%lluus",fd,off,n,e,
                (unsigned long long)(trace_now_us()-mmap_t0));
            free(p);
            errno=e;
            return(void*)-1;
        }
        if((size_t)got<n)trace_log_printf("MMAP","short file map fd=%d off=%ld req=%zu got=%lld",fd,off,n,(long long)got);
    }
    ...
    uint64_t mmap_us=trace_now_us()-mmap_t0;
    if((file_backed&&(n>=(256u*1024u)||mmap_us>=4000ull))||mmap_us>=TRACE_SLOW_IO_US||logical_size>=(64u*1024u*1024u))
        trace_log_printf("MMAP","map ptr=%p logical=%zu backing=%zu fd=%d off=%ld file=%d arena=%d time=%lluus",p,
            logical_size,backing_size,fd,off,file_backed,arena_index,(unsigned long long)mmap_us);
    return p;
}
...
typedef struct {
    ...
    CondVar cv;
    unsigned waiters;
    unsigned peak_waiters;
    uint64_t wait_calls;
    uint64_t wake_calls;
    uint64_t wake_total;
    uint64_t timeout_count;
    uint64_t compat_timeout_count;
    uint64_t last_waker_tid;
...
static long futex_operation(volatile int32_t *u, int op, int val, const struct timespec *to) {
    ...
    if (cmd == FUTEX_WAIT || cmd == FUTEX_WAIT_BITSET) {
        ...
        s->waiters++;
        s->wait_calls++;
        if(s->waiters>s->peak_waiters)s->peak_waiters=s->waiters;
        uint64_t wait_begin_us=trace_now_us();
        uint64_t wake_before=s->wake_total;
        Result wr = 0;
        guest_wait_begin("futex",(const void*)u);
        if (to) {
            ...
            /* The compatibility futex layer cannot guarantee Linux futex wake
             * ordering for every Unity atomic primitive. Preserve Android's
             * permitted spurious-wakeup behavior by rechecking every 16 ms. */
            wr = condvarWaitTimeout(&s->cv, &s->m, 16000000ull);
        }
        guest_wait_end();
        uint64_t wait_end_us=trace_now_us();
        uint64_t wake_after=s->wake_total;
        uint64_t last_waker=s->last_waker_tid;
        uint64_t last_wake=s->last_wake_us;
        if(R_FAILED(wr)){
            if(to)s->timeout_count++;
            else s->compat_timeout_count++;
        }
        s->waiters--;
        unsigned remaining=s->waiters;
        mutexUnlock(&s->m);

        uint64_t wait_us=wait_end_us>=wait_begin_us?wait_end_us-wait_begin_us:0;
        if(trace_is_main_thread()&&wait_us>=TRACE_MAIN_BLOCK_US){
            trace_log_printf("FUTEX","main key=%p wait=%lluus wake_delta=%llu remaining=%u last_waker_tid=%llu last_wake_age=%lluus",
                (void*)u,(unsigned long long)wait_us,(unsigned long long)(wake_after-wake_before),remaining,
                (unsigned long long)last_waker,(unsigned long long)(last_wake&&wait_end_us>=last_wake?wait_end_us-last_wake:0));
        }
        if (to && R_FAILED(wr)) {
    ...
    if (cmd == FUTEX_WAKE || cmd == FUTEX_WAKE_BITSET) {
        ...
        s->wake_calls++;
        s->wake_total+=wake_count;
        s->last_waker_tid=(uint64_t)(uint32_t)bionic_gettid();
        s->last_wake_us=trace_now_us();
...
void bionic_futex_diag_snapshot(const char *reason){
    typedef struct {
        void *key; unsigned waiters,peak;
        uint64_t waits,wake_calls,wake_total,timeouts,compat_timeouts,last_waker,last_wake_us;
    } Fsnap;
    Fsnap out[32]; unsigned n=0; uint64_t now=trace_now_us();
    ensure_bionic_state_initialized();
    mutexLock(&g_futex_registry_lock);
    for(unsigned i=0;i<FUTEX_SLOTS&&n<32;i++){
        FutexSlot *s=&g_futex_slots[i];
        if(!s->used)continue;
        mutexLock(&s->m);
        int interesting=s->waiters>0 || (s->last_wake_us&&now>=s->last_wake_us&&now-s->last_wake_us<5000000ull);
        if(interesting){
            Fsnap *x=&out[n++];
            x->key=(void*)s->key; x->waiters=s->waiters; x->peak=s->peak_waiters;
            x->waits=s->wait_calls; x->wake_calls=s->wake_calls; x->wake_total=s->wake_total;
            x->timeouts=s->timeout_count; x->compat_timeouts=s->compat_timeout_count;
            x->last_waker=s->last_waker_tid; x->last_wake_us=s->last_wake_us;
        }
        mutexUnlock(&s->m);
    }
    mutexUnlock(&g_futex_registry_lock);
    trace_log_printf("FUTEX","snapshot reason=%s count=%u",reason?reason:"?",n);
    for(unsigned i=0;i<n;i++){
        Fsnap *x=&out[i];
        trace_log_printf("FUTEX","key=%p waiters=%u peak=%u waits=%llu wake_calls=%llu wake_total=%llu timeouts=%llu compat_timeouts=%llu last_waker_tid=%llu last_wake_age=%llums",
            x->key,x->waiters,x->peak,(unsigned long long)x->waits,(unsigned long long)x->wake_calls,
            (unsigned long long)x->wake_total,(unsigned long long)x->timeouts,(unsigned long long)x->compat_timeouts,
            (unsigned long long)x->last_waker,(unsigned long long)(x->last_wake_us&&now>=x->last_wake_us?(now-x->last_wake_us)/1000ull:0));
    }
}
...
long bionic_syscall(long n,...){va_list a;va_start(a,n);long r=-1;
    ...
    else if(n==ARM64_SYS_SCHED_SETAFFINITY){long tid=va_arg(a,long);size_t sz=va_arg(a,size_t);const void*mask=va_arg(a,const void*);r=bionic_sched_setaffinity(tid,sz,mask);}
...
static long bionic_online_cpu_count(void){
    u64 mask=0;
    if(R_FAILED(svcGetInfo(&mask,InfoType_CoreMask,CUR_PROCESS_HANDLE,0))||!mask)return 1;
    unsigned count=0;
    while(mask){count+=(unsigned)(mask&1u);mask>>=1;}
    return count?count:1;
}
long bionic_sysconf(int name){
    switch(name){
        case 39:case 40:return 0x1000;
        case 96:case 97:return bionic_online_cpu_count();
        case 98:return (512ll*1024*1024)/0x1000;
        default:return -1;
    }
}
...
int bionic_pthread_mutex_lock(void*k){
    ...
    if(s->mutex_type==BIONIC_MUTEX_NORMAL){
        if(mutexTryLock(&s->m))return 0;
        guest_wait_begin("mutex",k);
        mutexLock(&s->m);
        guest_wait_end();
        return 0;
    }
...
static Result bionic_cond_wait_bridge(SyncSlot*cs,SyncSlot*ms,uint64_t timeout,int infinite){
    if(ms->mutex_type==BIONIC_MUTEX_NORMAL){
        if(infinite){condvarWait(&cs->cv,&ms->m);return 0;}
    ...
    if(ms->recursion_depth==1){
        ...
        Result r=0;
        if(infinite)condvarWait(&cs->cv,&ms->m);
        else r=condvarWaitTimeout(&cs->cv,&ms->m,timeout);
    ...
    mutexLock(&cs->m);
    Result r=0;
    if(infinite)condvarWait(&cs->cv,&cs->m);
    else r=condvarWaitTimeout(&cs->cv,&cs->m,timeout);
    mutexUnlock(&cs->m);
    ms->recursion_depth++;
    return r;
}
...
int bionic_pthread_cond_wait(void*c,void*m){
    ...
    guest_wait_begin("cond",c);
    Result r=bionic_cond_wait_bridge(cs,ms,BIONIC_COND_COOPERATIVE_NS,0);
    guest_wait_end();
    /* POSIX condition waits may wake spuriously. The short timeout is a
     * compatibility wake so Unity can re-check its predicate. */
...
int bionic_pthread_cond_timedwait(void*c,void*m,const struct timespec*t){
    ...
    if(rc)return rc;
    guest_wait_begin("cond_timed",c);
    Result r=bionic_cond_wait_bridge(cs,ms,timeout_ns,0);
    guest_wait_end();
    return bionic_cond_result(r,1);
}
...
int bionic_pthread_once(volatile int*o,void(*fn)(void)){
    if(__atomic_load_n(o,__ATOMIC_ACQUIRE)==2)return 0;
    int expected=0;
    if(__atomic_compare_exchange_n(o,&expected,1,0,__ATOMIC_ACQ_REL,__ATOMIC_ACQUIRE)){
        fn();
        __atomic_store_n(o,2,__ATOMIC_RELEASE);
    }else{
        guest_wait_begin("once",(const void*)o);
        while(__atomic_load_n(o,__ATOMIC_ACQUIRE)!=2)svcSleepThread(100000);
        guest_wait_end();
    }
    return 0;
}
...
static __thread GuestThread *g_current_guest_thread;
...
static GuestThread *guest_thread_find_tid_locked(uint64_t tid){
    uint64_t guest_tid=tid&0x7fffffffull;
    for(GuestThread *g=g_guest_threads;g;g=g->next)
        if((g->kernel_tid&0x7fffffffull)==guest_tid)return g;
    return NULL;
}

static long bionic_sched_setaffinity(long tid,size_t cpusetsize,const void *guest_mask){
    if(!guest_mask||cpusetsize==0){errno=EFAULT;return -1;}
    uint64_t requested=0;
    size_t copy=cpusetsize<sizeof(requested)?cpusetsize:sizeof(requested);
    memcpy(&requested,guest_mask,copy);

    u64 allowed=0;
    Result ir=svcGetInfo(&allowed,InfoType_CoreMask,CUR_PROCESS_HANDLE,0);
    if(R_FAILED(ir)||!allowed){errno=EINVAL;return -1;}

    Handle h=CUR_THREAD_HANDLE;
    uint64_t current_tid=(uint64_t)bionic_gettid();
    if(tid>0&&(uint64_t)tid!=current_tid){
        ensure_bionic_state_initialized();
        mutexLock(&g_thread_lock);
        GuestThread *g=guest_thread_find_tid_locked((uint64_t)tid);
        if(g)h=g->th.handle;
        mutexUnlock(&g_thread_lock);
        if(h==CUR_THREAD_HANDLE){
            trace_log_printf("SCHED","affinity target tid=%ld not found; keeping full mask=0x%llx",
                tid,(unsigned long long)allowed);
            return 0;
        }
    }

    /* Android CPU IDs/topology do not map to Tegra X1 CPU IDs. Keep guest
     * threads migratable across every core the homebrew process is allowed to
     * use, while preserving sched_setaffinity success semantics for Unity. */
    Result sr=svcSetThreadCoreMask(h,-1,(u32)allowed);
    trace_log_printf("SCHED","affinity tid=%ld requested=0x%llx applied=0x%llx rc=0x%x",
        tid,(unsigned long long)requested,(unsigned long long)allowed,(unsigned)sr);
    if(R_FAILED(sr)){errno=EINVAL;return -1;}
    return 0;
}


static inline void guest_wait_begin(const char *kind,const void *key){
    trace_main_block_begin(kind,key,NULL);
    GuestThread *g=g_current_guest_thread;
    if(!g)return;
    g->wait_key=(uintptr_t)key;
    g->wait_since_us=trace_now_us();
    __atomic_store_n(&g->wait_kind,kind,__ATOMIC_RELEASE);
}

static inline void guest_wait_end(void){
    trace_main_block_end();
    GuestThread *g=g_current_guest_thread;
    if(!g)return;
    __atomic_store_n(&g->wait_kind,NULL,__ATOMIC_RELEASE);
    g->wait_since_us=0;
    g->wait_key=0;
}
...
static void thread_entry(void *p){
    ...
    g_current_guest_thread=g;
    u64 ktid=0;
    if(R_SUCCEEDED(svcGetThreadId(&ktid,CUR_THREAD_HANDLE)))g->kernel_tid=ktid;
    s32 preferred=-1; u64 mask=0;
    (void)svcGetThreadCoreMask(&preferred,&mask,CUR_THREAD_HANDLE);
    trace_log_printf("THREAD","start handle=%08x tid=%llu core=%u preferred=%d mask=0x%llx name=%s",
        (unsigned)g->th.handle,(unsigned long long)g->kernel_tid,svcGetCurrentProcessorNumber(),preferred,
        (unsigned long long)mask,g->name[0]?g->name:"?");
    void *ret=g->fn(g->arg);
...
int bionic_pthread_create(uint64_t*out,const void*attr,void*(*fn)(void*),void*arg){
    ...
    u64 process_mask=0;
    Result mr=svcGetInfo(&process_mask,InfoType_CoreMask,CUR_PROCESS_HANDLE,0);
    if(R_SUCCEEDED(mr)&&process_mask){
        Result ar=svcSetThreadCoreMask(g->th.handle,-1,(u32)process_mask);
        if(R_FAILED(ar))
            trace_log_printf("SCHED","pthread_create core-mask failed handle=%08x rc=0x%x mask=0x%llx",
                (unsigned)g->th.handle,(unsigned)ar,(unsigned long long)process_mask);
    }else{
        trace_log_printf("SCHED","pthread_create process core-mask query failed rc=0x%x",(unsigned)mr);
    }
    ...
    *out=g->pthread_id;
    ...
    trace_log_printf("THREAD","create handle=%08x stack=%zuKB detached=%d process_mask=0x%llx",
        (unsigned)g->th.handle,stack>>10,g->detached,(unsigned long long)process_mask);
    return 0;
}
...
int bionic_pthread_join(uint64_t t,void**ret){
    ...
    guest_wait_begin("join",(const void*)(uintptr_t)t);
...
int bionic_pthread_setname_np(uint64_t t,const char*n){
    if(!n)return EINVAL;
    char name_copy[32]={0};
    int found=0;
    ensure_bionic_state_initialized();
    ...
    GuestThread *g=guest_thread_find_locked(t);
    if(g){
        snprintf(g->name,sizeof(g->name),"%.31s",n);
        snprintf(name_copy,sizeof(name_copy),"%s",g->name);
        found=1;
    }
    mutexUnlock(&g_thread_lock);
    if(found)trace_log_printf("THREAD","name handle=%08x name=%s",(unsigned)t,name_copy);
    return 0;
}
void bionic_thread_diag_snapshot(const char *reason){
    typedef struct {
        uint64_t pthread_id,kernel_tid;
        Handle handle;
        int detached,finished;
        char name[32];
        const char *wait_kind;
        uintptr_t wait_key;
        uint64_t wait_since_us;
        s32 preferred,priority;
        u64 mask;
    } Snap;
    Snap snap[64];
    unsigned n=0;
    ...
    ensure_bionic_state_initialized();
    mutexLock(&g_thread_lock);
    for(GuestThread *g=g_guest_threads;g&&n<64;g=g->next){
        Snap *x=&snap[n++];
        memset(x,0,sizeof(*x));
        x->pthread_id=g->pthread_id; x->kernel_tid=g->kernel_tid; x->handle=g->th.handle;
        x->detached=g->detached; x->finished=g->finished;
        snprintf(x->name,sizeof(x->name),"%s",g->name[0]?g->name:"?");
        x->wait_kind=__atomic_load_n(&g->wait_kind,__ATOMIC_ACQUIRE);
        x->wait_key=g->wait_key; x->wait_since_us=g->wait_since_us;
    }
    mutexUnlock(&g_thread_lock);

    trace_log_printf("THREAD","snapshot reason=%s count=%u",reason?reason:"?",n);
    for(unsigned i=0;i<n;i++){
        Snap *x=&snap[i];
        x->preferred=-1; x->priority=-1; x->mask=0;
        (void)svcGetThreadCoreMask(&x->preferred,&x->mask,x->handle);
        (void)svcGetThreadPriority(&x->priority,x->handle);
        uint64_t wait_ms=(x->wait_kind&&x->wait_since_us&&now>=x->wait_since_us)?(now-x->wait_since_us)/1000ull:0;
        trace_log_printf("THREAD","handle=%08x tid=%llu name=%s pref=%d mask=0x%llx prio=%d done=%d detached=%d wait=%s key=%p wait_ms=%llu",
            (unsigned)x->handle,(unsigned long long)x->kernel_tid,x->name,x->preferred,
            (unsigned long long)x->mask,x->priority,x->finished,x->detached,
            x->wait_kind?x->wait_kind:"run",(void*)x->wait_key,(unsigned long long)wait_ms);
    }
}

static void bionic_format_pc(uint64_t addr,char *out,size_t cap){
    const char *names[]={LIB_UNITY,LIB_IL2CPP,LIB_MAIN};
    const char *labels[]={"libunity","libil2cpp","libmain"};
    for(unsigned i=0;i<3;i++){
        so_module *m=so_find_named(names[i]);
        uintptr_t base=m?(uintptr_t)m->load_virtbase:0;
        if(base&&addr>=base&&addr<base+m->load_size){
            snprintf(out,cap,"%s+0x%llx",labels[i],(unsigned long long)(addr-base));
            return;
        }
    }
...
void bionic_thread_pc_snapshot(const char *reason,unsigned max_threads){
    typedef struct { Handle handle; uint64_t tid; char name[32]; } PcSnap;
    PcSnap snap[8]; unsigned n=0;
    if(max_threads>8)max_threads=8;
    ensure_bionic_state_initialized();
    mutexLock(&g_thread_lock);
    for(GuestThread *g=g_guest_threads;g&&n<max_threads;g=g->next){
        const char *wk=__atomic_load_n(&g->wait_kind,__ATOMIC_ACQUIRE);
        if(g->finished||wk)continue;
        if(g->name[0]&&!strncmp(g->name,"AAudio",6))continue;
        snap[n].handle=g->th.handle; snap[n].tid=g->kernel_tid;
        snprintf(snap[n].name,sizeof(snap[n].name),"%s",g->name[0]?g->name:"?");
        n++;
    }
    mutexUnlock(&g_thread_lock);
    for(unsigned i=0;i<n;i++){
        Result pr=svcSetThreadActivity(snap[i].handle,ThreadActivity_Paused);
        if(R_FAILED(pr)){
            trace_log_printf("PCSAMPLE","worker reason=%s tid=%llu name=%s pause_rc=0x%x",
                reason?reason:"?",(unsigned long long)snap[i].tid,snap[i].name,(unsigned)pr);
            continue;
        }
        ThreadContext ctx; memset(&ctx,0,sizeof(ctx));
        Result cr=svcGetThreadContext3(&ctx,snap[i].handle);
        Result rr=svcSetThreadActivity(snap[i].handle,ThreadActivity_Runnable);
        if(R_FAILED(cr)){
            trace_log_printf("PCSAMPLE","worker reason=%s tid=%llu name=%s context_rc=0x%x resume_rc=0x%x",
                reason?reason:"?",(unsigned long long)snap[i].tid,snap[i].name,(unsigned)cr,(unsigned)rr);
            continue;
        }
        char pc[64],lr[64]; bionic_format_pc(ctx.pc.x,pc,sizeof(pc)); bionic_format_pc(ctx.lr,lr,sizeof(lr));
        trace_log_printf("PCSAMPLE","worker reason=%s tid=%llu name=%s pc=%s lr=%s sp=0x%llx resume_rc=0x%x",
            reason?reason:"?",(unsigned long long)snap[i].tid,snap[i].name,pc,lr,
...
int bionic_sem_wait(void*k){
    ...
    if(!s){errno=ENOMEM;return -1;}
    mutexLock(&s->m);
    while(s->count<=0){guest_wait_begin("sem",k);condvarWait(&s->cv,&s->m);guest_wait_end();}
    s->count--;mutexUnlock(&s->m);return 0;
}

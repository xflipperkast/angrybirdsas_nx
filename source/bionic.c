...
void *bionic_mmap(void*a,size_t n,int prot,int flags,int fd,long off){
    ...
    int unity_dynamic=!file_backed&&(logical_size==(size_t)UNITY_DYNAMIC_RESERVE_LOGICAL);
    int unity_aligned256=!file_backed&&(logical_size==(size_t)UNITY_ALIGNED256_RESERVE_LOGICAL);
    int unity_reserve=unity_dynamic||unity_aligned256;
    unsigned arena_keep_count=unity_dynamic ? 1u :
                              unity_aligned256 ? UNITY_ALIGNED256_LIVE_SLOTS : 0u;

    unsigned arena_count=arena_keep_count ? arena_keep_count+1u : 0u;
    ...
    if(slot){
        ...
        slot->used=1;
        slot->unity_aligned_reserve=unity_reserve;
...
int bionic_munmap(void*p,size_t n){
    ...
    for(int i=0;i<MMAP_SLOTS;i++){
        ...
        if(start<base||start>=limit)continue;
        found=1;
